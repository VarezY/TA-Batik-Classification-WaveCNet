...
summary_acc1_train_avg = []
summary_acc5_train_avg = []
summary_acc1_test_avg = []
summary_acc5_test_avg = []
summary_train_loss_avg = []
summary_test_loss_avg = []
...
def main_worker(gpu, args, printer):
    ...
    # masukan summary from pyTorch
    printer.pprint('summary ==> {}'.format(torchinfo.summary(model, (1, 3, 224, 224), depth=3)))
    ...
    plot_data(data=summary_train_loss, filename='Train Loss')
    plot_data(data=summary_test_loss[1:], filename='Test Loss')
    plot_data(data=summary_acc1_test[1:], filename='Top 1 Acc Test')
    plot_data(data=summary_acc1_train, filename='Top 1 Acc Train')
    plot_data(data=summary_acc5_train, filename='Top 5 Acc Train')
    plot_data(data=summary_acc5_test[1:], filename='Top 5 Acc Test')
    plot_data(data=summary_train_loss_avg, filename='Train Avg Loss')
    plot_data(data=summary_test_loss_avg[1:], filename='Test Avg Loss')
    plot_data(data=summary_acc1_test_avg[1:], filename='Top 1 Avg Acc Test')
    plot_data(data=summary_acc1_train_avg, filename='Top 1 Avg Acc Train')
    plot_data(data=summary_acc5_train_avg, filename='Top 5 Avg Acc Train')
    plot_data(data=summary_acc5_test_avg[1:], filename='Top 5 Avg Acc Test')
...
def train(train_loader, model, criterion, optimizer, epoch, args, printer, start_time):
    ...
    summary_acc1_train.append(top1.val.cpu().numpy())
    summary_acc5_train.append(top5.val.cpu().numpy())
    summary_train_loss.append(losses.val)

    summary_acc1_train_avg.append(top1.avg.cpu().numpy())
    summary_acc5_train_avg.append(top5.avg.cpu().numpy())
    summary_train_loss_avg.append(losses.avg)
...
def validate(val_loader, model, criterion, args, printer, start_time = None):
    ...
    summary_acc1_test.append(top1.val.cpu().numpy())
    summary_acc5_test.append(top5.val.cpu().numpy())
    summary_test_loss.append(losses.val)

    summary_acc1_test_avg.append(top1.avg.cpu().numpy())
    summary_acc5_test_avg.append(top5.avg.cpu().numpy())
    summary_test_loss_avg.append(losses.avg)
...
def plot_data(data, filename=None):
    ...
    plt.grid(axis='both', color='0.7', linestyle=':', linewidth=1.3)
    plt.xlabel("Epoch (n)")
    plt.title(filename)
    ...
    if filename:
        ...
        plt.savefig(str(random.randint(0, 99)) + '.png')
    plt.clf()
